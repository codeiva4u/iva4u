...
class MovieRulzUPN : ExtractorApi() {
    ...
    override val mainUrl = "https://movierulz2025.upns.pro"
    override val requiresReferer = true
    ...
    override suspend fun getUrl(
        ...
        doc.selectFirst("script:containsData(function(p,a,c,k,e,d))")?.data()?.let { packed ->
            JsUnpacker(packed).unpack()?.let { unpacked ->
                Regex("""file:"(.*?)"""").find(unpacked)?.groupValues?.get(1)?.let { m3u8Link ->
                    callback.invoke(
                        newExtractorLink(
                            name,
                            name,
...
class Streamsn : ExtractorApi() {
    ...
    override suspend fun getUrl(
        ...
        doc.selectFirst("script:containsData(function(p,a,c,k,e,d))")?.data()?.let { packed ->
            JsUnpacker(packed).unpack()?.let { unpacked ->
                Regex("file:\\s*\"(https?://[^\"]+\\.m3u8[^\"]*)\"").find(unpacked)?.groupValues?.get(1)?.let { m3u8Link ->
                    callback.invoke(
                        newExtractorLink(
                            name,
                            name,
