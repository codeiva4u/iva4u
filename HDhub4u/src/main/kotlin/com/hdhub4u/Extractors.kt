...
open class Hblinks : ExtractorApi() {
    ...
    override suspend fun getUrl(
        ...
        app.get(newUrl).documentLarge.select("h3 a,h5 a,div.entry-content p a").map {
            ...
            when {
                ...
                else -> Log.d("Hblinks", "Skipping unknown link: $href")
            }
        }
    }
}
...
class Hubdrive : ExtractorApi() {
    ...
    override suspend fun getUrl(
        ...
        val href = app.get(newUrl, timeout = 5000L).documentLarge
        ...
        when {
            ...
            href.contains("hdstream4u", ignoreCase = true) -> {
                HdStream4u().getUrl(href, "HubDrive", subtitleCallback, callback)
            }
            href.contains("hubstream", ignoreCase = true) -> {
                Hubstream().getUrl(href, "HubDrive", subtitleCallback, callback)
            }
            else -> {
                Log.d("Hubdrive", "Skipping unknown link: $href")
            }
        }
    }
}
        ...
        document.select("a.btn").forEach { element ->
            ...
            when {
                ...
                "10gbps" in label -> {
                    ...
                    // Skip unknown links - only use project extractors
                    Log.d(tag, "Skipping unknown link: $link")
                }
            }
        }
    }
