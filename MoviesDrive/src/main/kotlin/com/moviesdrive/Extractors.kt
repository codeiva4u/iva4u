...
// ═══════════════════════════════════════════════════════════════════
// MDrive Extractor - Scrapes mdrive.lol pages and calls sub-extractors
// ═══════════════════════════════════════════════════════════════════
class MDriveExtractor : ExtractorApi() {
    override val name: String = "MDrive"
    override val mainUrl: String = "https://mdrive.lol"
    override val requiresReferer = false
        ...
        try {
            ...
            val downloadLinks = document.select("a[href]").mapNotNull { element ->
                ...
                when {
                    ...
                    href.contains("gdflix", ignoreCase = true) -> Pair("gdflix", href)
                    href.contains("gdlink", ignoreCase = true) -> Pair("gdlink", href)
                    href.contains("gofile", ignoreCase = true) -> Pair("gofile", href)
                    else -> null
                }
            }
            ...
            downloadLinks.amap { (type, link) ->
                try {
                    when (type) {
                        "hubcloud" -> {
                            Log.d("MDriveExtractor", "Calling HubCloud for: $link")
                            HubCloud().getUrl(link, referer, subtitleCallback, callback)
                        }
                        "gdflix", "gdlink" -> {
                            Log.d("MDriveExtractor", "Calling GDFlix for: $link")
                            GDFlix().getUrl(link, referer, subtitleCallback, callback)
                        }
                        "gofile" -> {
                            Log.d("MDriveExtractor", "Calling Gofile for: $link")
                            Gofile().getUrl(link, referer, subtitleCallback, callback)
                        }
                    }
                } catch (e: Exception) {
                    Log.e("MDriveExtractor", "Error extracting $type link: ${e.message}")
                }
            }
        } catch (e: Exception) {
            Log.e("MDriveExtractor", "Error fetching mdrive.lol page: ${e.message}")
        }
    }
}
