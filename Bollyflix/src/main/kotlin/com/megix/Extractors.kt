...
class LinksMod : ExtractorApi() {
    ...
    override suspend fun getUrl(
        ...
        try {
            ...
            Log.e("LinksMod", "Error: ${e.message}")
        }
    }
}

class SideXFee : ExtractorApi() {
    override val name = "SideXFee"
    override var mainUrl = "https://web.sidexfee.*"
    override val requiresReferer = false
    ...
    override suspend fun getUrl(
        ...
        try {
            ...
            if (encodeUrl.isNotBlank()) {
                val decoded = base64Decode(encodeUrl.replace("\\/", "/"))
                if (decoded.isNotBlank()) {
                    loadExtractor(decoded, url, subtitleCallback, callback)
                }
            }
        } catch (e: Exception) {
            Log.e("SideXFee", "Error: ${e.message}")
        }
    }
}
