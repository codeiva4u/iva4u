...
class Bolly4uProvider : MainAPI() {
    ...
    companion object {
        const val TAG = "Bolly4u"

        private val YEAR_REGEX = Regex("""\b(19\d{2}|20\d{2})\b""")

        private val SERIES_DETECTION_REGEX = Regex(
            """(?i)\b(Season|S\d{1,2}|Episodes?|EP\d{1,3}|Series|Complete|All\s*Episodes)\b"""
        )
        ...
        override var mainUrl: String = "https://bolly4u.org"
            ...
            if (cachedDomain != null && System.currentTimeMillis() - lastFetchTime < 3600000) {
                ...
                try {
                    kotlinx.coroutines.withTimeoutOrNull(2_000L) {
                        ...
                        val urlString = jsonObject.optString("bolly4u")
    ...
    override suspend fun getMainPage(
        ...
        val anchors = document.select("article a, .post-title a, div.post-thumbnail a, h2 a, h3 a").toList().ifEmpty {
            document.select("a[href*='bolly4u']").toList()
        }
    ...
    private fun parseFileSize(text: String): Double {
        val match = Regex("""([\d.]+)\s*(GB|MB)""", RegexOption.IGNORE_CASE).find(text) ?: return 0.0
    ...
    private suspend fun extractDownloadLinks(document: Document): List<DownloadLink> {
        ...
        for (link in downloadLinks) {
            if (link.url.contains("fxlinks", ignoreCase = true) ||
                link.url.contains("fastdlserver", ignoreCase = true) ||
