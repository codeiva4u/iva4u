...
fun getServerPriority(serverName: String): Int {
    return when {
        ...
        serverName.contains("Cloud", true) -> 700
        serverName.contains("R2", true) -> 680
...
fun shouldBlockUrl(url: String): Boolean {
    val blockList = listOf(
        ...
        "telegram.org", "telegram.me", "filesgram.xyz",
        "linksmod.top/register", "linksmod.top/login", "linksmod.top/save",
        "1fichier.com"
    )
...
fun isInvalidLink(url: String, text: String): Boolean {
    ...
    if (lowerText.contains("telegram") || lowerText.contains("login") || lowerText.contains("linksmod") || lowerText.contains("tutorial") || lowerText.contains("how to")) return true
    return false
}
    ...
    override suspend fun getUrl(
        ...
        try {
            ...
            if (finalUrl.isNotBlank() && finalUrl != url) {
                ...
                val innerLink = doc.selectFirst("a[href*='gdflix'], a[href*='hubcloud'], a[href*='fastdl'], a[href*='filebee'], a[href*='gofile']")?.attr("href") ?: ""
...
open class LinksMod : ExtractorApi() {
    ...
    override suspend fun getUrl(
        ...
        try {
            val doc = app.get(url).document
            doc.select("a[href^='http']").amap { element ->
                val abs = element.absUrl("href")
                val href = if (abs.isNotBlank()) abs else element.attr("href")
                if (href.isBlank() || href.startsWith("#") || href.contains("linksmod.top")) return@amap
                if (shouldBlockUrl(href)) return@amap

                when {
                    href.contains("gofile.io", true) || href.contains("multiup", true) || href.contains("goflix.sbs", true) -> {
                        processPluginExtractor(href, referer, subtitleCallback, callback)
                    }
                    href.contains("vikingfile", true) || href.contains("megaup", true) -> {
                        callback(newExtractorLink(
                            "LinksMod", "LinksMod [Download]", href
                        ) {
                            this.quality = Qualities.Unknown.value
                            this.headers = VIDEO_HEADERS
                        })
                    }
                    href.endsWith(".mkv", true) || href.endsWith(".mp4", true) ||
                    href.contains("r2.dev", true) || href.contains("r2.cloudflarestorage.com", true) -> {
                        processPluginExtractor(href, referer, subtitleCallback, callback)
                    }
                }
            }
        } catch (e: Exception) {
            Log.e(tag, "Error processing linksmod: ${e.message}")
        }
    }
}
...
suspend fun processPluginExtractor(
    ...
    try {
        when {
            ...
            link.contains("goflix.sbs", true) || link.contains("multiup", true) -> {
                try {
                    val mirrorDoc = app.get(link).document
                    val gofileUrl = mirrorDoc.selectFirst("a[href*='gofile.io']")?.attr("href") ?: ""
                    if (gofileUrl.isNotBlank()) {
                        Gofile().getUrl(gofileUrl, referer, subtitleCallback, callback)
                    }
                } catch (e: Exception) {
                    Log.e("Multiup", "Mirror error: ${e.message}")
                }
            }
            ...
            link.contains("workers.dev", true) ||
            link.contains("busycdn", true) ||
            link.contains("pages.dev", true) ||
            link.endsWith(".mkv", true) ||
            link.endsWith(".mp4", true) -> {
                val cleanUrl = if (link.contains("url=")) {
    ...
    override suspend fun getUrl(
        ...
        try {
            ...
            document.select("div.text-center a, a.btn").forEach { anchor ->
                ...
                try {
                    when {
                        text.contains("Instant DL", true) || href.contains("instant.", true) || href.contains("busycdn", true) -> {
                            val finalLink = try {
                                val resp = app.get(href, allowRedirects = true)
                                val finalUrl = resp.url
                                when {
                                    finalUrl.contains("url=") ->
                                        URLDecoder.decode(finalUrl.substringAfter("url="), StandardCharsets.UTF_8.toString())
                        ...
                        text.contains("CLOUD DOWNLOAD", true) || href.contains("r2.dev", true) || href.contains("r2.cloudflarestorage.com", true) -> {
                            val r2Link = if (href.contains("url="))
                                URLDecoder.decode(href.substringAfter("url="), "UTF-8") else href
                            if (r2Link.isNotBlank() && !r2Link.contains(".zip")) {
                                callback(newExtractorLink(
                        ...
                        text.contains("FAST CLOUD", true) || text.contains("ZIPDISK", true) || href.contains("/cloud/", true) -> {
                            try {
                                ...
                                if (cloudHref.isNotBlank()) {
                                    val cleanVideoUrl = cloudHref
                                        ...
                                        .replace(".mp4.zip", ".mp4")
                                        .replace(Regex("""\.zip(\?|$)"""), "$1")
                                    if (shouldBlockUrl(cleanVideoUrl)) return@forEach
                                    callback(newExtractorLink(
                        ...
                        text.contains("GOFILE", true) || href.contains("goflix.sbs", true) || href.contains("gofile", true) || href.contains("multiup", true) -> {
                            try {
                        ...
                        text.contains("DIRECT DL", true) || text.contains("DIRECT SERVER", true) || href.contains("indexserver", true) || href.contains("thunder.", true) -> {
                            callback(newExtractorLink(
                                "GDFlix[Direct]", "GDFlix[Direct] $fileName [$fileSize]", href
                            ) {
