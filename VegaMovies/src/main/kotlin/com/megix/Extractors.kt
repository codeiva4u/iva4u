...
class Nexdrive : ExtractorApi() {
    override val name: String = "Nexdrive"
    override val mainUrl: String = "https://nexdrive.*"
    ...
    override suspend fun getUrl(
        ...
        try {
            ...
            doc.select("a[href*='fastdl.zip'], a[href*='vcloud.zip'], a[href*='filebee.xyz'], a[href*='vcloud'], a[href*='hubcloud']").forEach { element ->
                val link = element.attr("href")
                if (link.isNotBlank()) {
                    if (link.contains("vcloud")) {
                        VCloud().getUrl(link, url, subtitleCallback, callback)
                    } else {
            ...
            Log.e("Nexdrive", "Error: ${e.message}")
        }
    }
}
