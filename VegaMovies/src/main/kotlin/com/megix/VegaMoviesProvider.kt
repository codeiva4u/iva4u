        ...
        if (tvtype == "series") {
            ...
            for (tag in hTags) {
                ...
                Eurl?.let { eurl ->
                    ...
                    val vcloudLinks = document2.select("p > a, div.entry-inner a").mapNotNull {
                        val href = it.attr("href")
                        if (href.contains("vcloud", true) || href.contains("nexdrive", true) || href.contains("fastdl.zip", true) || href.contains("filebee", true)) {
                            href
                        } else {
                            null
                        }
            ...
            val buttons = document.select("a:has(button.dwd-button), a[href*='nexdrive']")
            val data = buttons.mapNotNull { button ->
                val link = fixUrl(button.attr("href"))
                val doc = app.get(link).document
                var source = doc.select("a[href*='vcloud'], a[href*='fastdl.zip'], a[href*='filebee']").attr("href")
                if (source.isBlank()) {
                    source = link
                }
